...
export default function App() {
  ...
  const resetAll = () => {
    setAnswers(
      Object.fromEntries(questions.map((q) => [q.id, null])) as AnswerMap
    );
    setSubmitted(false);
  };
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 16px" }}>
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "#6b7280" }}>
            0/1/2点 × {questions.length}問（最大 {questions.length * 2}点）
          </div>
        </header>
          ...
          {questions.map((q, idx) => (
            <li key={q.id} style={card}>
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: "4px 0 10px 0", fontWeight: 600 }}>
                    {q.text}
                  </p>
                  <div
                    style={{
        ...
        >
          <button
            ...
            style={{ ...btn, opacity: unanswered > 0 ? 0.6 : 1 }}
          ...
          <button onClick={copyResult} style={btnGhost}>
            結果をコピー
          </button>
        </div>
        ...
        {submitted && (
          <section style={{ ...card, marginTop: 16 }}>
            ...
            <div style={{ fontSize: 14, color: "#4b5563", marginBottom: 8 }}>
              ...
              {questions.length * 2}（カットオフ {cutoff} 点）
            </div>
            <div
              style={{
                border: "1px solid #e5e7eb",
